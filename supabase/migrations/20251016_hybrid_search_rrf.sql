...
-- First, ensure we have a text search configuration on chunks
CREATE INDEX IF NOT EXISTS chunks_content_fts_idx 
ON chunks 
USING gin(to_tsvector('english', content));

-- Hybrid search function using RRF
...
COMMENT ON FUNCTION search_chunks_hybrid IS 'Hybrid search using Reciprocal Rank Fusion (RRF) - combines semantic vector search with BM25 keyword search';
